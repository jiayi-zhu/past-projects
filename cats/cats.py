...
def pick(paragraphs, select, k):
    ...
    """
    # BEGIN PROBLEM 1
    selected_lst = [elem for elem in paragraphs if select(elem)]
    if k >= len(selected_lst):
        return ''
    else:
        return selected_lst[k]
    # END PROBLEM 1
...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    def abouc_func(x):
        lst = split(remove_punctuation(lower(x)))
        for elem in lst:
            if elem in topic:
                return True
        return False
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        return typed_word
    my_lst = [diff_function(typed_word,elem,limit) for elem in word_list]
    if min(my_lst) > limit:
        return typed_word
    min_index = my_lst.index(min(my_lst))
...
def hidden_kittens(typed, reference, limit):
    ...
    def helper(typed,reference):
        if not reference:
            return 1
        elif reference[0] not in typed:
            return 0
        match_index = typed.index(reference[0])
        return helper(typed[match_index + 1:], reference[1:]) + \
                helper(typed[match_index + 1:],reference)
    result = helper(typed,reference)
    return limit + 1 if not result or result > limit else result
    # END PROBLEM 7
...
def report_progress(typed, prompt, user_id, upload):
    ...
    """
    # BEGIN PROBLEM 8
    count, first_incorrect, i= 0, False, 0
    while i < len(typed) and not first_incorrect:
        if typed[i] == prompt[i]:
            count += 1
        else:
...
def time_per_word(words, times_per_player):
    ...
    for player in times_per_player:
        ...
        for i in range(len(player)-1):
            time_lst.append(player[i+1] - player[i])
        player_lst.append(time_lst)
    return match(words,player_lst)
    # END PROBLEM 9
...
def fastest_words(match):
    ...
    for w in word_indices:
        ...
        for p in player_indices:
            lst.append(match['times'][p][w])
        fastest_player_index = lst.index(min(lst))
        result[fastest_player_index].append(match["words"][w])
    return result
    # END PROBLEM 10
