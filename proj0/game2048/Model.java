...
public class Model extends Observable {
    ...
    public void col_move(int c){
        int topRow = _board.size() - 1;
        Tile topTile = _board.tile(c,topRow);
        for (int r = _board.size() - 2; r >= 0; r -= 1){
            Tile t = _board.tile(c, r);
            if (topTile == null && t!= null) {
                _board.move(c, topRow, t);
                topTile = _board.tile(c,topRow);
            } else if (t != null && topTile != null){
                if (t.value() == topTile.value()){
                    _board.move(c,topRow,t);
                    topRow -= 1;
                    topTile = _board.tile(c,topRow);
                    _score += t.value() * 2;
