...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8b
            assert (place.ant.is_container and place.ant.can_contain(self)) or (self.is_container and self.can_contain(place.ant)), 'Two ants in {0}'.format(place)
    ...
    def double(self):
        ...
        if self.is_container and self.ant_contained and not self.ant_contained.has_doubled:
            self.ant_contained.damage *= 2
            self.ant_contained.has_doubled = True
        else:
            return
        # END Problem 12
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        distance = 0
        curr = self.place
        while self.lower_bound > distance:
            if curr.is_hive:
                return
            curr = curr.entrance
            distance += 1
        while distance < self.upper_bound and not curr.bees and not curr.is_hive:
            curr = curr.entrance
            distance += 1
        if curr.is_hive or not curr.bees:
            return
        else:
...
class HungryAnt(Ant):
    ...
    def action(self,gamestate):
        if self.turns_to_chew > 0:
            ...
            if selected_bee:
                selected_bee.reduce_health(selected_bee.health)
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        #the same as Place.add_insect(self,insect)
...
# END Problem 12
    ...
    def construct(cls, gamestate):
        ...
        if not gamestate.has_queen:
            constructed = super().construct(gamestate)
            if constructed:
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        destination_scared = self.place.entrance

        # Extra credit: Special handling for bee direction
        if not self.is_scared and not self.slowed:
            if self.blocked():
            ...
            if gamestate.time % 2 == 1:
                ...
                if self.blocked():
                    ...
                    if self.is_scared:
                        if not destination_scared.is_hive:
                            self.move_to(destination_scared)
                            self.scared_turns -= 1
                        else:
                            self.scared_turns -= 1
            ...
            if self.blocked():
                self.sting(self.place.ant)
            elif self.health > 0 and not destination_scared.is_hive:
                self.move_to(destination_scared)
                self.scared_turns -= 1
            elif destination_scared.is_hive:
                self.scared_turns -= 1
    ...
    def scare(self, length):
        ...
        """
        # BEGIN Problem EC
        if not self.already_scared:
            self.already_scared = True
            self.is_scared = True
