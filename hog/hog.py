...
def announce_lead_changes(score0, score1, last_leader=None):
    ...
    if score0 > score1:
        ...
        curr_leader = None
    if curr_leader != None and curr_leader != last_leader:
        score_differ = max(score0,score1) - min(score0,score1)
        message = f"Player {curr_leader} takes the lead by {score_differ}"
        leader = curr_leader
    else:
        message = None
        leader = curr_leader
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    """
    # BEGIN PROBLEM 9
    num_roll, max_value, max_index = 1, 1, 1
    while num_roll <= 10:
        averaged_dice = make_averaged(roll_dice, total_samples)
        score = averaged_dice(num_roll,dice)
        if score > max_value:
            max_index = num_roll
            max_value = score
        num_roll += 1
